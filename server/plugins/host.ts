...
//  - 通知: announce は HTTP を介さず announceToDiscord(db, text) を直接呼ぶ。
...
import { loadPlugins, resolveExternalPluginDirs } from './memoria-plugin/host/loader.js';
import { mountPlugins } from './memoria-plugin/host/registry.js';
import { createInProcessMemoriaClient } from './memoria-plugin/host/memoria-client.js';
...
/**
 * 同梱 + 外部 (MEMORIA_PLUGINS_DIR) のプラグインを読み込み、 本体 app に /plugins/<id>
 * でマウントする。 manifest (相対 url) を返す → /api/plugins で UI に渡す。
 */
export async function mountUserApps(
  ...
  const external = resolveExternalPluginDirs();
  const plugins = await loadPlugins([BUNDLED_PLUGINS_DIR, ...external]);
  const memoria = createInProcessMemoriaClient((text) => announceToDiscord(db, text));
  // publicBaseUrl 省略 → manifest url は /plugins/<id> の相対 (同一オリジン iframe 用)。
  return mountPlugins(app, plugins, {
