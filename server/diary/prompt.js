...
export const WORK_CONTENT_PROMPT = ({ dateStr, urlList, activityList, totalEvents, totalDomains, activityCounts }) => [
...
export function composeSummary({ workContent, githubByRepo, highlights, digs, activity }) {
  const parts = [];
  ...
  if (activity && activity.total > 0) {
    ...
    const lines = sorted.map((it) => {
      ...
      const tag = it.kind === 'git_commit' ? 'git' : it.kind === 'claude_code_prompt' ? 'cc' : it.kind;
...
export function buildActivityList(activity) {
...
export function formatActivityCounts(activity) {
