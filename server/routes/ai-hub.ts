...
export function makeAiHubRouter(deps: AiHubRouterDeps): Hono {
  ...
  r.post('/api/ai/articles/:id/transcribe', async (c: Context) => {
    ...
    const result = await importToTabula({
      title: (article.title || 'AI 記事').slice(0,200), tags: [...new Set(['ai-article',...article.tags.map(tag=>`${tag.category}:${tag.value}`.slice(0,64))])].slice(0,32), source_kind: 'ai_article', source_ref: String(article.id),
      elements: [{block_type:'text',text:article.body_md}],
    });
    setAiArticleNote(db, id, result.note.id);
