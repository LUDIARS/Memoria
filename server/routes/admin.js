...
export function createAdminRouter({
  ...
  findGpsLocationById,
  GPS_STATIONARY_THRESHOLD_M,
  // external visits
  insertExternalVisitEvent,
  // activity events
  recordActivityEvent,
  activityEventsForDate,
  listActivityEvents,
  activityEventsPage,
  // app settings + LLM
  getAppSettings,
  ...
  router.post('/locations/compress', async (c) => {
    let body = {};
  ...
  // ---- GPS place (resolve-debug / resolve-all) --------------------------------

  router.get('/locations/unresolved', (c) => {
    const limit = Math.min(Number(c.req.query('limit') ?? 50), 200);
    return c.json({ items: listUnresolvedGpsLocations(db, limit) });
  });

  router.post('/locations/resolve-all', async (c) => {
    const items = listUnresolvedGpsLocations(db, 100);
    return c.json({ queued: items.length, items: items.map(r => r.id) });
  });

  // ---- Tracks settings --------------------------------------------------------

  router.get('/tracks/settings', (c) => {
  ...
  router.patch('/tracks/settings', async (c) => {
    const body = await c.req.json().catch(() => ({}));
    const patch = {};
  ...
  // ---- External visits (Legatus DNS/SNI tap) ----------------------------------

  router.get('/visits/external/stats', (c) => {
    const c24 = db.prepare(`SELECT COUNT(*) AS n FROM visit_events WHERE source IN ('dns','sni') AND visited_at >= datetime('now','-1 day')`).get()?.n ?? 0;
    const c7  = db.prepare(`SELECT COUNT(*) AS n FROM visit_events WHERE source IN ('dns','sni') AND visited_at >= datetime('now','-7 days')`).get()?.n ?? 0;
    const dev = db.prepare(`SELECT COUNT(DISTINCT device_label) AS n FROM visit_events WHERE source IN ('dns','sni') AND device_label IS NOT NULL AND visited_at >= datetime('now','-7 days')`).get()?.n ?? 0;
    const latest = db.prepare(`SELECT visited_at FROM visit_events WHERE source IN ('dns','sni') ORDER BY visited_at DESC LIMIT 1`).get()?.visited_at ?? null;
    const recent = db.prepare(`SELECT visited_at, domain, device_label, device_os, source FROM visit_events WHERE source IN ('dns','sni') ORDER BY visited_at DESC LIMIT 20`).all();
    return c.json({ count_24h: c24, count_7d: c7, device_count: dev, latest, recent });
  });

  router.post('/visits/external', async (c) => {
    const body = await c.req.json().catch(() => null);
  ...
  router.get('/external/stats', (c) => {
    const g24 = db.prepare(`SELECT COUNT(*) AS n FROM gps_locations WHERE recorded_at >= datetime('now','-1 day')`).get()?.n ?? 0;
    const g7  = db.prepare(`SELECT COUNT(*) AS n FROM gps_locations WHERE recorded_at >= datetime('now','-7 days')`).get()?.n ?? 0;
    const gDev = db.prepare(`SELECT COUNT(DISTINCT device_id) AS n FROM gps_locations WHERE device_id IS NOT NULL AND recorded_at >= datetime('now','-7 days')`).get()?.n ?? 0;
    const gLatest = db.prepare(`SELECT recorded_at FROM gps_locations ORDER BY recorded_at DESC LIMIT 1`).get()?.recorded_at ?? null;
    const gKey = (getAppSettings(db)['locations.ingest_key'] || '').trim();
    const gKeyConfigured = !!gKey || !!process.env.LOCATIONS_INGEST_KEY;
    const d24 = db.prepare(`SELECT COUNT(*) AS n FROM visit_events WHERE source IN ('dns','sni') AND visited_at >= datetime('now','-1 day')`).get()?.n ?? 0;
    const d7  = db.prepare(`SELECT COUNT(*) AS n FROM visit_events WHERE source IN ('dns','sni') AND visited_at >= datetime('now','-7 days')`).get()?.n ?? 0;
    const dDev = db.prepare(`SELECT COUNT(DISTINCT device_label) AS n FROM visit_events WHERE source IN ('dns','sni') AND device_label IS NOT NULL AND visited_at >= datetime('now','-7 days')`).get()?.n ?? 0;
    const dLatest = db.prepare(`SELECT visited_at FROM visit_events WHERE source IN ('dns','sni') ORDER BY visited_at DESC LIMIT 1`).get()?.visited_at ?? null;
    const dRecent = db.prepare(`SELECT visited_at, domain, device_label, device_os, source FROM visit_events WHERE source IN ('dns','sni') ORDER BY visited_at DESC LIMIT 20`).all();
    return c.json({
      location: { configured: gKeyConfigured, active: g24 > 0, count_24h: g24, count_7d: g7, device_count: gDev, latest: gLatest },
      dns: { configured: d7 > 0, active: d24 > 0, count_24h: d24, count_7d: d7, device_count: dDev, latest: dLatest, recent: dRecent },
    });
  });

  // ---- Activity events --------------------------------------------------------

  router.post('/activity/event', async (c) => {
    let body;
