...
| 0-11 | Cernere 側 `managed_projects.frontend_origin` 拡張依頼 | `Cernere/` 側 issue (別 PR) | Phase 0 で issue だけ open |
...
(c) `frontend_origin` の更新で検出する。 既存 entry の probe は manifest schema
が薄いため毎回行わず、 上記トリガ + 6 時間毎のローテーション probe のみ。

### 9.3 Cernere 側の要件

Cernere の `managed_projects` には現状 `frontend_origin` 相当のフィールドが
無いため、 拡張が必要 (実作業は `Cernere/spec/project-management.md` の
別 issue で扱う):

- `managed_projects.frontend_origin TEXT NULL` — そのプロジェクトの
  frontend / Hub 公開元 URL (例: `https://bibliotheca.example.com`)
- `managed_project.list` の戻り値に `frontend_origin` を含める
- `frontend_origin` が NULL のプロジェクトは Hub Shell の同期対象外
  (= manifest を持たない方針のプロジェクト、 例: 純 backend サービス)

### 9.4 probe 仕様

Hub backend が走らせる manifest probe の規格:

- **エンドポイント**: `GET <frontend_origin>/.well-known/ludiars-app.json`
...
- `source = 'cernere'` — Cernere 由来、 §9.2 の自動同期対象、 元プロジェクトが
  非 active / `frontend_origin` 削除になったら自動 disable
...
- Cernere 側 `managed_projects.frontend_origin` 拡張 issue を Cernere repo に open (依存)
