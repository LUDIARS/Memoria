...
## データ
- [work_locations](../data/workplace.md) — name / address / latitude / longitude / description / url / tags / shareable / shared_at / owner_user_id
- 集計入力: [gps_locations](../data/gps.md), [activity_events](../data/activity.md)
- 状態: `app_settings.workplace.current.id` / `workplace.current.at` (現在チェックイン中の workplace)

## API
- [workplace.md](../interface/workplace.md) — `/api/work-locations*` (CRUD) / `/api/work-locations/resolve-place` / `/api/work-locations/checkin` / `/api/work-sessions` (1 日のセッション検出)
- 関連: [multi.md](../interface/multi.md) `/api/multi/share` (kind=work_location)、 Hub への presence は `shareWorkplacePresence` 経由 (multi-client.js)
