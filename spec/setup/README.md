...
| Maps key (SPA 用) | app_settings `maps.api_key` (env からは読まない) | `server/routes/config.ts:306` |
| Places key (server 側) | app_settings `maps.api_key` (env からは読まない) | `server/lib/place-resolver.ts:100-104` |
