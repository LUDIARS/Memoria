...
- **「ログイン時に自動起動」** をトレイメニューから ON にすると、
  ...
  またはダブルクリックで開けます。
- ログアウト / シャットダウン中は サーバも止まります。 これは仕様で、
  **「Memoria が起動していない時間 = ユーザは作業していない時間」**
  と扱える設計上の利点があります (アクセス記録の欠落を 「データなし」
  ではなく 「離席 / 作業外」 として解釈できる)。 Windows サービス化や
  タスクスケジューラでログアウト中も動かす方向には踏み込みません。
